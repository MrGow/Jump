    ...
    if (fade_state == 1) { // fading out
        ...
        if (fade_alpha <= 0) {
            ...
            if (instance_exists(target)) {
                ...
                if (variable_instance_exists(target, "vsp")) target.vsp = 0;
            }

            if (debug_cam) {
                var _zid = instance_exists(active_zone) ? string(active_zone) : "noone";
                show_debug_message("FADE END zone=" + _zid + " cam=(" + string(cam_logic_x) + "," + string(cam_logic_y) + ")");
            }
        }
    }
}
