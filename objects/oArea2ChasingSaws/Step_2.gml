...
//
// This keeps retrying every frame while saw_loop_id is
// noone. It solves direct room testing where oGame begins
// loading the group during the same frame.
// ----------------------------------------------------
if (
    saw_loop_id == noone &&
    saw_loop_asset != -1 &&
...
// ----------------------------------------------------
// Horizontal distance from saw to player
// ----------------------------------------------------
...
{
    // ------------------------------------------------
    // Temporary movement farther into the screen
    // ------------------------------------------------
    if (burst_active)
    {
    ...
    // ------------------------------------------------
    // Follow the left side of the camera
    // ------------------------------------------------
    x =
        ctrl.cam_x +
        screen_offset_x +
...
//
// This is deliberately performed after burst movement.
// If the burst ends above, the next burst can be detected
// properly when a later trigger activates it.
// ----------------------------------------------------
