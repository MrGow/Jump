...
sprite_index = spriteFloorSawSaw;

// Only blade pixels should exist in this mask.
// Same canvas size + same origin as spriteFloorSawSaw.
mask_index = spriteSawBladeMask;
...
snd_saw_loop = asset_get_index("SingleSawLoop1");

saw_emitter = audio_emitter_create();
saw_sound_instance = -1;
saw_current_gain = 0;
saw_audio_allowed = false;
...
find_patrol_point = function()
{
    ...
    if (!patrol_enabled) return;
    if (string(patrol_id) == "") return;

    var point_count = instance_number(oSawPatrolPoint);

    for (var i = 0; i < point_count; i++)
    {
        var pt = instance_find(oSawPatrolPoint, i);

        if (pt == noone) continue;

        if (
            variable_instance_exists(pt, "enabled")
            && !pt.enabled
        )
        {
            continue;
        ...
        if (
            variable_instance_exists(pt, "patrol_id")
            && string(pt.patrol_id) == string(patrol_id)
        )
        {
            patrol_point = pt;
            patrol_end_x = pt.x;
...
apply_facing = function()
{
    facing_direction = clamp(round(facing_direction), 0, 3);

    switch (facing_direction)
    {
        case 0: image_angle = 0;   break;
        case 1: image_angle = 270; break;
        case 2: image_angle = 180; break;
...
move_to_target = function(_tx, _ty)
{
...
saw_is_audio_candidate = function(_player)
{
    if (_player == noone) return false;

    var my_dist = point_distance(x, y, _player.x, _player.y);
    ...
    for (var oi = 0; oi < array_length(saw_objects); oi++)
    {
        var saw_obj = saw_objects[oi];
        var count = instance_number(saw_obj);

        for (var si = 0; si < count; si++)
        {
            var other_saw = instance_find(saw_obj, si);

            if (other_saw == noone || other_saw == id)
            {
                continue;
            }

            if (
                variable_instance_exists(other_saw, "enabled")
                && !other_saw.enabled
            )
            {
                continue;
            ...
            var other_outer =
                variable_instance_exists(other_saw, "saw_sound_outer_dist")
            ...
            var definitely_closer = other_dist < my_dist - 0.001;

            var tied_but_wins =
                abs(other_dist - my_dist) <= 0.001
                && other_saw.id < id;

            if (definitely_closer || tied_but_wins)
            {
                closer_count++;

                if (closer_count >= saw_sound_max_simultaneous)
                {
                    return false;
                }
            }
...
stop_saw_audio = function()
{
    ...
    {
        audio_stop_sound(saw_sound_instance);
    }
